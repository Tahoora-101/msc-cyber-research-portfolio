# msc-cyber-research-portfolio

# 🔒 Cybersecurity Research Portfolio  

> A living portfolio documenting my technical, research, and policy journey toward graduate studies in Cybersecurity & Strategic Studies.  
> Structured as a **tick-off tracker** for my 10 core milestones.  

---

## ✅ Progress Tracker (0–10 Goals)

| # | Goal | Status | 
|---|------|--------|
| 0 | Setup: GitHub, Documentation Standards | 🟢 Done | 
| 1 | **Linux Foundations** → OTW Bandit | 🟢 Active | 
| 2 | **Python Security** → FreeCodeCamp String Manipulation | 🟢 Active | 
| 3 | **Cipher Projects** → Caesar, Vigenère, XOR | ⚪ Planned | 
| 4 | **Networking Mastery** → CompTIA Network+ Notes & Labs | 🟢 Active | 
| 5 | **Security Fundamentals** → CompTIA Security+ Notes | ⚪ Planned | 
| 6 | **Applied Research Report** → Dell Inspiron Stability Analysis | 🟡 Researching 
| 7 | **Capture the Flag** → PicoCTF / HTB Writeups | ⚪ Planned | 
| 8 | **Formal Research Paper** → Academic-style technical publication | 🔴 Later | 
| 9 | **Policy Integration** → Cyber Policy Digests & Comparative Papers | 🟢 Active | 
| 10 | **Portfolio Polishing** → 2 major public repos (Linux + Networking) | 🔴 Later |
| 11 | **Scholarship Readiness** → CV + Proposal + Supervisor Mapping | 🔴 Later | 

---

## 🏗️ Portfolio Structure  

### 🔧 Technical Skills  
Hands-on cybersecurity foundations.  
- Linux Security & Hardening 🐧  
- Python for Security (ciphers, automation) 🐍  
- Networking & Defense 🌐  
- CTF Problem-Solving 🎯  

### 🔬 Research Projects  
Evidence of academic methodology.  
- Dell Inspiron Stability Analysis (hardware root cause)  
- Vulnerability & malware research drafts  
- Research methodology notes  

### 📊 Policy Research  
Bridging tech & strategy.  
- Cyber Policy Digests (weekly summaries)  
- Comparative cybersecurity strategies (US–SG–EU)  
- Regional cyber governance analysis  

---

## 🚀 Featured Work  

| Project | Area | Status | Summary |
|---------|------|--------|---------|
| [OTW Bandit Walkthrough](./1-TECHNICAL/Linux/OTW-Bandit.md) | Linux | 🟢 Active | Linux fundamentals & privilege escalation basics |
| [Python Ciphers](./1-TECHNICAL/Python-Ciphers/) | Python Security | 🟢 Active | String Manipulation |
| [Dell Inspiron Case Report](./2-RESEARCH/Dell-Inspiron-Case/) | Research | 🟡 Planning | Hardware/driver incompatibility investigation |
| [Cyber Policy Digest](./3-POLICY/Digests/) | Policy | 🟢 Active | Weekly notes linking technical issues to governance |

---

## 🎯 Research Focus  

- IoT & Embedded Security 🔌  
- Network Security & Monitoring 🌐  
- Vulnerability Research 🔍  
- Cyber Policy & Governance 📜  
- Security Automation 🤖  

---

## 📈 Learning Journey  

- **Certifications:** CompTIA Network+ (ongoing), CompTIA Security+ (planned)  
- **Skill Paths:** Advanced Linux, Python Automation, Research Writing, Policy Analysis  

---

<div align="center">

*“Strategy without tactics is the slowest route to victory. Tactics without strategy is the noise before defeat.”* – Sun Tzu  

**Last Updated**: 29-09-2025  

</div>

